    ...
    fun start() {
        thread {
            try {
                embeddedServer(Netty, port = AppConfig.SERVER_PORT) {
                    routing {
                        ...
                        post("/api/action") {
                            ...
                            Logger.log("Ação $action em ${source.name}: $success")
                        ...
                        post("/upload") {
                            ...
                            call.receiveMultipart().forEachPart { part ->
                                if (part is PartData.FileItem) {
                                    ...
                                    Logger.log("Upload concluído: ${f.name}")
                                }
                                part.dispose()
                            }
                ...
                Logger.log("CRITICAL SERVER ERROR: ${e.message}")
            }
        }
    }
}